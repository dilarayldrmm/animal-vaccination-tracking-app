import { useState } from 'react';
import {
  View,
  Text,
  TextInput,
  TouchableOpacity,
  StyleSheet,
  Alert,
  Platform,
} from 'react-native';
import DateTimePicker from '@react-native-community/datetimepicker';
import { addVaccineToCat } from '../services/storageService';
import { scheduleVaccineNotifications } from '../services/notificationService';
export default function AddVaccineScreen({ route, navigation }) {
  const cat = route.params?.cat;

  const [vaccineName, setVaccineName] = useState('');
  const [vaccineDate, setVaccineDate] = useState(null);
  const [showDatePicker, setShowDatePicker] = useState(false);

  const handleDateChange = (event, selectedDate) => {
    if (Platform.OS === 'android') {
      setShowDatePicker(false);
    }

    if (selectedDate) {
      setVaccineDate(selectedDate);
    }
  };

  const handleSave = async () => {
    if (!vaccineName || !vaccineDate) {
      Alert.alert('Eksik Bilgi', 'Aşı adı ve tarih seçmelisin.');
      return;
    }

    const vaccine = {
      id: Date.now().toString(),
      name: vaccineName.trim(),
      date: vaccineDate.toISOString(),
    };

    const updatedCat = await addVaccineToCat(cat.id, vaccine);

     await scheduleVaccineNotifications(cat.name, vaccine.name, vaccine.date);
    Alert.alert('Başarılı', 'Aşı kaydedildi.');
    navigation.navigate('Home', { cat: updatedCat });
  };

  return (
    <View style={styles.container}>
      <Text style={styles.title}>Aşı Ekle</Text>
      <Text style={styles.subtitle}>{cat?.name} için yeni aşı tarihi gir</Text>

      <View style={styles.card}>
        <Text style={styles.label}>Aşı Adı</Text>
        <TextInput
          placeholder="Örn: Kuduz Aşısı"
          value={vaccineName}
          onChangeText={setVaccineName}
          style={styles.input}
        />

        <Text style={styles.label}>Aşı Tarihi</Text>
        <TouchableOpacity
          style={styles.dateInput}
          onPress={() => setShowDatePicker(true)}
        >
          <Text style={vaccineDate ? styles.dateText : styles.placeholderText}>
            {vaccineDate
              ? vaccineDate.toLocaleDateString('tr-TR')
              : 'Takvimden tarih seç'}
          </Text>
        </TouchableOpacity>

        {showDatePicker && (
          <DateTimePicker
            value={vaccineDate || new Date()}
            mode="date"
            display={Platform.OS === 'ios' ? 'spinner' : 'default'}
            onChange={handleDateChange}
          />
        )}

        <TouchableOpacity style={styles.saveButton} onPress={handleSave}>
          <Text style={styles.saveButtonText}>Aşıyı Kaydet</Text>
        </TouchableOpacity>
      </View>
    </View>
  );
}

const styles = StyleSheet.create({
  container: {
    flex: 1,
    backgroundColor: '#FDF2E9',
    padding: 22,
  },
  title: {
    fontSize: 30,
    fontWeight: '800',
    color: '#3D2B1F',
    textAlign: 'center',
    marginTop: 25,
  },
  subtitle: {
    fontSize: 15,
    color: '#7A6A5E',
    textAlign: 'center',
    marginTop: 8,
    marginBottom: 25,
  },
  card: {
    backgroundColor: '#FFFFFF',
    borderRadius: 24,
    padding: 20,
  },
  label: {
    fontSize: 14,
    fontWeight: '700',
    color: '#3D2B1F',
    marginBottom: 8,
    marginTop: 10,
  },
  input: {
    backgroundColor: '#FAFAFA',
    borderWidth: 1,
    borderColor: '#E5D6C8',
    borderRadius: 14,
    padding: 14,
    fontSize: 15,
    marginBottom: 10,
  },
  dateInput: {
    backgroundColor: '#FAFAFA',
    borderWidth: 1,
    borderColor: '#E5D6C8',
    borderRadius: 14,
    padding: 14,
    marginBottom: 10,
  },
  dateText: {
    color: '#3D2B1F',
    fontSize: 15,
  },
  placeholderText: {
    color: '#999',
    fontSize: 15,
  },
  saveButton: {
    backgroundColor: '#E67E22',
    paddingVertical: 16,
    borderRadius: 16,
    alignItems: 'center',
    marginTop: 18,
  },
  saveButtonText: {
    color: '#FFFFFF',
    fontSize: 16,
    fontWeight: '800',
  },
});